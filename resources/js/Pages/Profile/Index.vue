...
import { Bookmark, Ellipsis, ExternalLink, Eye, FileEdit, Grid3X3, Heart, Lock, MapPin, Megaphone, MessageCircle, Pencil, Play, Repeat2, Settings, Share2, Tag, Trash2, X } from '@lucide/vue';
...
const promotePost = (item: any) => { window.location.assign(`/sponsorship?video=${encodeURIComponent(item.id)}`); };
        ...
        <main class="self-profile">
            <section class="self-profile-header">
                ...
                <div class="self-profile-copy">
                    ...
                    <div class="self-actions"><Link href="/profile/edit">Edit profile</Link><Link v-if="profile?.roles?.includes('athlete')" href="/profile/statistics">Career</Link><Link href="/profile/gallery">Library</Link><Link href="/sponsorship?create=post">Promote post</Link><Link class="round" href="/settings/devices" aria-label="Settings"><Settings :size="18" /></Link><button class="round" aria-label="Share profile"><Share2 :size="18" /></button></div>
            ...
            <Transition name="profile-preview">
                <div v-if="controlsPost" class="profile-post-actions-overlay" @click.self="controlsPost = null">
                    <section class="profile-post-actions" role="dialog" aria-modal="true" aria-label="Post actions">
                        ...
                        <button v-if="controlsPost.status === 'published'" type="button" @click="promotePost(controlsPost)"><Megaphone :size="17" /><span><strong>Promote post</strong><small>Choose an audience and campaign budget</small></span></button>
...
.profile-post-tags { display: flex; align-items: center; gap: .3rem; overflow: hidden; color: #476FEA; font-size: .66rem; white-space: nowrap; }
...
.profile-preview-hint { display: flex; align-items: center; gap: .3rem; color: #476FEA; font-size: .65rem; font-weight: 750; }
...
.post-preview-copy { overflow-y: auto; padding: 1.4rem; }.post-preview-copy h2 { margin: 0 0 .8rem; font-size: 1.1rem; line-height: 1.45; }.post-preview-tags { display: flex; flex-wrap: wrap; gap: .4rem; color: #476FEA; font-size: .78rem; }
...
.post-preview-comments { display: grid; gap: .75rem; margin-top: 1.25rem; padding-top: 1rem; border-top: 1px solid #edf0f5; }.post-preview-comments > div:first-child { display: flex; align-items: center; justify-content: space-between; gap: .75rem; }.post-preview-comments h3 { margin: 0; font-size: .9rem; }.post-preview-comments a { display: inline-flex; align-items: center; gap: .3rem; color: #476FEA; font-size: .7rem; font-weight: 750; }.preview-comment { display: flex; gap: .6rem; }.preview-comment > span { display: grid; flex: 0 0 32px; width: 32px; height: 32px; place-items: center; color: #fff; border-radius: 50%; background: linear-gradient(135deg,#476FEA,#e83e95); font-size: .62rem; font-weight: 800; }.preview-comment strong { font-size: .72rem; }.preview-comment p { margin: .15rem 0; color: #3d495c; font-size: .72rem; line-height: 1.4; }.preview-comment small { color: #7b8798; font-size: .62rem; }.preview-comment-state { margin: 0; color: #7b8798; font-size: .72rem; }.preview-comment-state.error { color: #c62828; }
