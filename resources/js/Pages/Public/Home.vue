...
const props = defineProps<{ highlights: any[] }>();
...
const playPreview = (event: MouseEvent) => { const video = (event.currentTarget as HTMLElement).querySelector('video'); if (!video) return; if (!video.src && video.dataset.src) video.src = video.dataset.src; video.play().catch(() => undefined); };
...
onMounted(() => { startTimer(); const loadMedia = () => { mediaReady.value = true; window.setTimeout(() => heroVideo.value?.play().catch(() => undefined), 50); }; if ('requestIdleCallback' in window) window.requestIdleCallback(loadMedia, { timeout: 1400 }); else window.setTimeout(loadMedia, 700); }); onBeforeUnmount(() => window.clearInterval(timer));
</script>
...
<div class="kinetic-media" :class="{ 'is-paused': paused }"><video v-if="current?.video" :key="current.id" ref="heroVideo" :src="mediaReady ? current.video : undefined" :poster="current.cover" :autoplay="mediaReady" :muted="muted" loop playsinline preload="none"/><img v-else :src="current?.cover || '/images/homepage-hero.png'" alt="Athletes competing across multiple sports" fetchpriority="high"/></div><div class="kinetic-wash"/><div class="kinetic-light" aria-hidden="true"/><div class="kinetic-orbit orbit-one" aria-hidden="true"/><div class="kinetic-orbit orbit-two" aria-hidden="true"/>
