...
const feed = computed(() => props.mode === 'following' ? (props.videos ?? []) : props.videos?.length ? props.videos : demos);
const featured = computed(() => feed.value[0]);
const trending = computed(() => props.suggestions?.length ? props.suggestions.slice(0, 4) : demos.map(item => item.creator).slice(0, 4));
...
<template>
    ...
    <AppShell>
                ...
                <div class="feed-stream">
                    <div v-for="(item, index) in feed" :id="item.id" :key="item.id" class="featured-video-wrap">
                        ...
                        <div class="featured-actions">
                            ...
                            <button :class="{ active: item.viewer?.liked }" @click="interact(item,'like')"><span><Heart /></span><small>{{ compact(item.counts.likes) }}</small></button>
                            ...
                            <button :class="{ active: item.viewer?.saved }" @click="interact(item,'save')"><span><Bookmark /></span><small>{{ compact(item.counts.saves) }}</small></button>
        ...
        <Transition name="gate">
            <div v-if="controlsPost" class="action-sheet-overlay" @click.self="controlsPost = null"><section class="feed-action-sheet"><header><h2>Post controls</h2><button @click="controlsPost = null"><X /></button></header><div class="sheet-list"><button @click="reportPost(controlsPost)"><Flag />Report</button><button @click="hidePost(controlsPost)"><X />Not interested</button><a v-if="controlsPost.url" :href="controlsPost.url" download><Download />Download</a><Link href="/sponsorship"><Megaphone />Promote</Link><button @click="castPost(controlsPost)"><Cast />Cast</button><button @click="interact(controlsPost,'save'); controlsPost = null"><Bookmark />Save</button></div></section></div>
        </Transition>
        <Transition name="gate">
