...
const ownStoryIndex = computed(() => stories.value.findIndex(story => story.viewer?.is_owner));
const followedStories = computed(() => stories.value.filter(story => !story.viewer?.is_owner));
...
<template>
    ...
    <AppShell>
        <div class="for-you-page">
            ...
            <section v-if="authenticated && !location && !sportFilter && !positionFilter" class="stories-rail" aria-label="Stories"><button v-if="ownStoryIndex>=0" class="story-circle own-story" @click="openStory(ownStoryIndex)"><span><img v-if="stories[ownStoryIndex].creator.profile_image" :src="stories[ownStoryIndex].creator.profile_image" alt="Your Story"/><b v-else>{{stories[ownStoryIndex].creator.name.slice(0,2).toUpperCase()}}</b></span><strong>Your Story</strong><small>View</small></button><Link v-else href="/upload?type=story" class="story-circle own-story"><span><Plus/></span><strong>Your Story</strong></Link><template v-if="mode==='following'"><button v-for="story in followedStories" :key="story.id" class="story-circle" @click="openStory(stories.indexOf(story))"><span><img v-if="story.creator.profile_image" :src="story.creator.profile_image" :alt="story.creator.name"/><b v-else>{{story.creator.name.slice(0,2).toUpperCase()}}</b></span><strong>{{story.creator.name}}</strong></button></template></section>
