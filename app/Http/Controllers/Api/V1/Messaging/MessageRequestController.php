...
class MessageRequestController extends Controller
{
    ...
    public function store(StoreMessageRequestRequest $request, CreateMessageRequest $create, NotificationDispatcher $notifications): JsonResponse
    {
        ...
        $notifications->send($recipient, 'message_requests', ['event' => 'message_request_received', 'request_id' => $messageRequest->public_id, 'actor_id' => $request->user()->id, 'actor_name' => $request->user()->name]);
    ...
    public function accept(MessageRequest $messageRequest, RespondToMessageRequest $respond, NotificationDispatcher $notifications): JsonResponse
    {
        Gate::authorize('respond', $messageRequest);
        $result = $respond->execute($messageRequest, true);
        $notifications->send($result->sender, 'message_requests', ['event' => 'message_request_accepted', 'request_id' => $result->public_id, 'conversation_id' => $result->conversation->public_id, 'actor_id' => $result->recipient_id, 'actor_name' => $result->recipient->name]);
