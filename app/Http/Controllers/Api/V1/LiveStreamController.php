...
class LiveStreamController extends Controller
{
    ...
    {
        $item = DB::table('live_streams')->where('public_id', $stream)->where('status', 'live')->first();
        abort_unless($item, 404);
        DB::table('live_streams')->where('id', $item->id)->increment('viewer_count');
        DB::table('live_streams')->where('id', $item->id)->update(['peak_viewers' => DB::raw('MAX(peak_viewers, viewer_count)')]);
        broadcast(new LiveActivity($stream, ['type' => 'viewer_join', 'count' => $item->viewer_count + 1, 'viewer_id' => $request->user()->id]));

        return response()->json(['data' => ['viewer_count' => $item->viewer_count + 1]]);
    }
