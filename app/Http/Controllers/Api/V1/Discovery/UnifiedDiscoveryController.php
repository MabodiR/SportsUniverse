...
class UnifiedDiscoveryController extends Controller {
    public function __invoke(Request $request):JsonResponse {
        ...
        $user=$request->user();$term=trim((string)$request->query('q'));$searchTerm=ltrim($term,'#');$like='%'.mb_strtolower($searchTerm).'%';$city=$user->profile?->city;
        ...
        $videos=Video::query()->where('status','published')->where('visibility','public')->when($term,fn($q)=>$q->where(fn($b)=>$b->whereRaw('LOWER(caption) LIKE ?',[$like])->orWhereRaw('LOWER(CAST(hashtags AS TEXT)) LIKE ?',[$like])->orWhereRaw('LOWER(location_name) LIKE ?',[$like])->orWhereHas('user',fn($u)=>$u->whereRaw('LOWER(name) LIKE ?',[$like]))))->with('user.profile','images')->latest('published_at')->limit(8)->get()->map(fn($v)=>['id'=>$v->public_id,'caption'=>$v->caption,'hashtags'=>$v->hashtags??[],'location'=>$v->location_name,'creator'=>['name'=>$v->user->name,'slug'=>$v->user->profile?->slug],'thumbnail'=>$v->images->first()?route('media.public',$v->images->first()):null,'stream'=>$v->media_id?route('videos.stream',$v):null,'views'=>$v->views_count]);
        ...
        // Trending is intentionally sampled from the newest posts. Loading every
        // video from the last 14 days can exhaust memory once the feed contains
        // millions of records, while a recent sample keeps discovery responsive.
        $recent=Video::where('status','published')->where('visibility','public')->where('published_at','>=',now()->subDays(14))->latest('published_at')->limit(10_000)->pluck('hashtags')->flatMap(fn($v)=>$v??[])->map(fn($v)=>mb_strtolower($v))->countBy()->sortDesc()->take(12)->map(fn($count,$tag)=>['tag'=>$tag,'posts'=>$count])->values();
        $nearbyAthletes=!$term&&$city?User::whereKeyNot($user->id)->whereHas('profile',fn($q)=>$q->where('is_public',true)->whereRaw('LOWER(city)=?',[mb_strtolower($city)]))->with('roles','profile','athleteProfile.sport','organisationProfile')->limit(8)->get()->map(fn($p)=>$this->profile($p)):collect();
        $nearbyOpportunities=!$term&&$city?Opportunity::where('status','published')->whereRaw('LOWER(city)=?',[mb_strtolower($city)])->with('poster','sport')->limit(8)->get()->map(fn($o)=>['id'=>$o->public_id,'title'=>$o->title,'city'=>$o->city,'sport'=>$o->sport?->name]):collect();
        $recommended=collect();if(!$term){$followed=$user->following()->pluck('users.id');$recommended=User::whereKeyNot($user->id)->whereNotIn('id',$followed)->whereHas('profile',fn($q)=>$q->where('is_public',true))->with('roles','profile','athleteProfile.sport','organisationProfile')->withCount('followers')->orderByDesc('followers_count')->limit(8)->get()->map(fn($p)=>$this->profile($p));}
