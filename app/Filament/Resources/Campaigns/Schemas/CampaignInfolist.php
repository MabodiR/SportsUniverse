...
class CampaignInfolist
{
    public static function configure(Schema $schema): Schema
    {
        return $schema->components([
            ...
            TextEntry::make('spent_cents')->label('Amount spent')->money('ZAR', divideBy: 100),
            TextEntry::make('impressions_count')->label('Impressions')->numeric(),
            TextEntry::make('clicks_count')->label('Clicks')->numeric(),
            TextEntry::make('click_rate')->label('Click-through rate')->state(fn ($record) => $record->impressions_count ? round($record->clicks_count / $record->impressions_count * 100, 2).'%' : '0%'),
