...
export default function ConversationScreen() {
  ...
  const upload = async (asset: { uri: string; name: string; type: string }, kind: 'image' | 'video' | 'document') => { setUploading(true); try { const form = new FormData(); form.append('kind', kind); form.append('collection', 'uploads'); form.append('file', { uri: asset.uri, name: asset.name, type: asset.type } as any); let media = (await api.post<ApiResponse<MediaUpload>>('/media', form, { headers: { 'Content-Type': 'multipart/form-data' }, timeout: 600000 })).data.data; for (let attempt = 0; attempt < 30 && media.processing_status !== 'ready'; attempt++) { await new Promise(resolve => setTimeout(resolve, 1500)); media = (await api.get<ApiResponse<MediaUpload>>('/media/' + media.id)).data.data; if (media.processing_status === 'failed') throw new Error(media.processing_error || 'Attachment processing failed.'); } if (media.processing_status !== 'ready') throw new Error('Attachment is still processing.'); send.mutate({ media_id: media.id }); } catch (error) { Alert.alert('Attachment not sent', errorMessage(error)); } finally { setUploading(false); } };
