...
export default function UploadScreen() {
    ...
    const pick = async (kind: "image" | "video") => {
        ...
        if (!permission.granted)
            return Alert.alert(
                ...
                "Allow photo-library access in device settings to choose media.",
            );
        ...
        const invalid = result.assets.find(
            (asset) =>
                (asset.fileSize ?? 0) >
                (kind === "image" ? 10 * 1024 * 1024 : 500 * 1024 * 1024),
        );
        if (invalid)
            return Alert.alert(
                "File too large",
        ...
        if (kind === "video") {
            const seconds = Math.max(
                1,
                Math.floor((result.assets[0].duration ?? 60000) / 1000),
            );
            setTrimStart("0");
            setTrimLength(String(Math.min(60, seconds)));
        }
    };
    ...
    const publish = () =>
        Alert.alert(
            "Finish your post",
            "Publish it now or keep it private as a draft?",
            [
    ...
    const submit = async (publishNow: boolean) => {
        if (!assets.length)
            return Alert.alert(
                "Choose media",
                "Select at least one photo or one video.",
            );
        setBusy(true);
        setProgress(0);
        ...
        try {
            ...
            for (let index = 0; index < assets.length; index++) {
                ...
                if (kind === "video") {
                    const seconds = Math.max(
                        1,
                        Math.floor((asset.duration ?? 60000) / 1000),
                    );
                    ...
                    const length = Math.min(
                        60,
                        Math.max(1, Number(trimLength) || 60),
                        seconds - start,
                    );
                    form.append(
                        "trim_start_ms",
                    ...
                    form.append(
                        "trim_end_ms",
                        String(Math.round((start + length) * 1000)),
                    );
                }
                form.append("file", {
                    uri: asset.uri,
                ...
                const response = await api.post<ApiResponse<MediaUpload>>(
                    "/media",
                    form,
                    {
                        timeout: 600000,
            ...
            Alert.alert(
                publishNow ? "Post not published" : "Draft not saved",
                errorMessage(error),
            );
        } finally {
            setBusy(false);
        ...
        return (
            <SafeAreaView style={styles.safe}>
                ...
                <View style={styles.center}>
                    <Text style={styles.heading}>Sign in to upload.</Text>
                    <PrimaryButton
                        label="Sign in"
                ...
                >
                    ...
                    {assets[0]?.type === "video" ? (
                        <View style={styles.trimCard}>
                            <Text style={styles.switchTitle}>Trim video</Text>
                            <Text style={styles.switchCopy}>
                                Choose any segment up to 60 seconds from the
                                original video.
                            </Text>
                    ...
                    <TextInput
                        ...
                        placeholder="Tell the story. Add #hashtags..."
                    ...
                    <TextInput
                        ...
                        placeholder="Stadium, city or training ground"
                        placeholderTextColor="#71849B"
                        style={styles.input}
                    />
                    ...
                    <View style={styles.switchRow}>
                        <View>
                            ...
                            <Text style={styles.switchCopy}>
                                Members can respond to your post.
                            </Text>
                        </View>
                        <Switch
                            value={comments}
...
function Picker({
    ...
    return (
        <Pressable onPress={onPress} style={styles.picker}>
            <Ionicons name={icon} size={25} color="#79A3FF" />
...
const styles = StyleSheet.create({
    ...
    pickers: { flexDirection: "row", gap: 11, marginTop: 24 },
    picker: {
        flex: 1,
        height: 86,
        alignItems: "center",
    ...
    pickerText: { color: colors.white, fontSize: 13, fontWeight: "900" },
    previews: { gap: 9, marginTop: 14 },
    preview: {
        width: 105,
        height: 105,
    ...
    caption: { height: 130, paddingTop: 14 },
    counter: {
        color: colors.muted,
        fontSize: 10,
        textAlign: "right",
    ...
    switchTitle: { color: colors.white, fontSize: 13, fontWeight: "900" },
    switchCopy: { color: colors.muted, fontSize: 10, marginTop: 3 },
    progress: { marginTop: 20 },
    progressTop: { flexDirection: "row", justifyContent: "space-between" },
    phase: { color: colors.white, fontSize: 11, fontWeight: "800" },
    percent: { color: "#79A3FF", fontSize: 11, fontWeight: "900" },
    track: {
        height: 7,
        overflow: "hidden",
        borderRadius: 4,
