...
export function FeedCard({ video, height, onProtectedAction, index, active, authenticated }: { video: Video; height: number; onProtectedAction: () => void; index: number; active: boolean; authenticated: boolean }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => api.post('/videos/' + video.id + '/views', { watched_ms: 1000, completed: false }).then(response => { setCounts(current => ({ ...current, views: response.data.data.views_count })); if (video.sponsored?.delivery_id) api.post('/campaign-deliveries/' + video.sponsored.delivery_id + '/conversion', { event: 'video_view' }).catch(() => undefined); }).catch(() => undefined), 1000);
    return () => clearTimeout(timer);
  }, [active, authenticated, video.id, video.sponsored?.delivery_id]);
  useEffect(() => {
    if (!active || !video.sponsored?.delivery_id) return;
    const timer = setTimeout(() => api.post('/campaign-deliveries/' + video.sponsored!.delivery_id + '/impression').catch(() => undefined), 1000);
    return () => clearTimeout(timer);
  }, [active, video.sponsored?.delivery_id]);
  ...
  const follow = async () => {
    ...
    try {
      ...
      if (!following && video.sponsored?.delivery_id) api.post('/campaign-deliveries/' + video.sponsored.delivery_id + '/conversion', { event: 'follow' }).catch(() => undefined);
  ...
  const share = async () => {
    ...
    const result = await Share.share({ message: (video.caption ?? 'SportsUniverse highlight') + '\n' + url, url });
  ...
  const openSponsored = async () => {
    if (!video.sponsored?.delivery_id) return;
    await api.post('/campaign-deliveries/' + video.sponsored.delivery_id + '/click').catch(() => undefined);
    if (video.sponsored.goal === 'followers') await api.post('/campaign-deliveries/' + video.sponsored.delivery_id + '/conversion', { event: 'profile_visit' }).catch(() => undefined);
    if (video.sponsored.destination_url) Linking.openURL(video.sponsored.destination_url).catch(() => undefined);
  };
  const toggleSound = () => {
